...
# Global Constants
SPEED = 0.36    # game difficulty
SNAKE_SIZE = 9
APPLE_SIZE = SNAKE_SIZE    
SEPARATION = 10    # separation between two pixels
SCREEN_HEIGHT = 600
SCREEN_WIDTH = 800
FPS = 25    # game difficulty
KEY = {"UP":1 , "DOWN":2 , "LEFT":3, "RIGHT":4}


# Initializing screen
screen = pygame.display.set_mode((SCREEN_WIDTH,SCREEN_HEIGHT),pygame.HWSURFACE)
# HWSURFACE (Hardware Surface) refers to using memory on the video card for storing draws as opposed to main memory
...
# the snake can pass through one side of screen and come from other side
...
# creating class for Apple (Snake's food)
class Apple:
...
class snake:
    ...
    # For each apple that snake eats, the snake will grow so for that adding that apple to stack
    def grow(self):
        last_element = len(self.stack) - 1
    ...
    # Defining Game Over Function when Snake eats itself
    ...
    for event in pygame.event.get():
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_UP:
                ...
                return KEY["RIGHT"]
            elif event.key == pygame.K_ESCAPE:      # if want to exit the game(press esc)
                return "exit"
            elif event.key == pygame.K_y:       # if want to play again(press Y)
                return "yes"
            elif event.key == pygame.K_n:       # if don't want to play again(press N)
                return "no"
...
def drawGameTime(gameTime):
    ...
    screen.blit(game_time_numb,(115,14))


# Defining function  to randomly respawn apple
...
def main():
    ...
    mySnake.move()
    start_segments = 4   # initially snake will have 4 segments long excluding head
    ...
    # Initializing apple
    max_apples = 1         # max apple on screen
    eaten_apple = False   # as snake will eat apple, it will disappear
